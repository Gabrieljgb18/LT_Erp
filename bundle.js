	                            ...
	                            <table class="table table-hover table-sm align-middle mb-0">
	                                <thead class="table-light">
	                                    <tr>
	                                        ...
	                                        <th class="py-2 text-muted font-weight-normal">Asistencia</th>
    ...
    function renderSummary(invoices) {
        ...
        const pageItems = rows.slice(start, start + PAGE_SIZE);
	        ...
	        pageItems.forEach(item => {
	            const tr = document.createElement('tr');
	            const asistencia = item && item.asistencia === false ? false : true;
	            const badge = asistencia
	                ? '<span class="badge text-bg-success">Sí</span>'
	                : '<span class="badge text-bg-secondary">No</span>';
	            const horasRegistradas = (item && item.horasRegistradas != null) ? item.horasRegistradas : (item ? item.horas : 0);
	            const horasFact = (item && item.horasFacturables != null) ? item.horasFacturables : (item ? item.horas : 0);
	            const horasCell = asistencia
	                ? `<strong>${horasRegistradas || 0}</strong>`
	                : `<span class="text-muted">${horasRegistradas || 0}</span><div class="small text-muted">Facturable: ${horasFact || 0}</div>`;
	            tr.innerHTML = `
